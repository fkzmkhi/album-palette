...
export default function TopAlbumCard({ album, index, onClick, onPaletteExtracted }: Props) {
	...
	return (
		...
		>
			<div className="relative aspect-square w-full overflow-hidden rounded-3xl bg-zinc-200 transition group-hover:-translate-y-0.5">
			...
			<div className="min-w-0">
				<p className="truncate text-sm font-medium text-zinc-900 leading-snug">{album.name}</p>
				<p className="truncate text-xs text-zinc-500 mt-0.5">{album.artist}</p>
			</div>
		</button>
	);
}
