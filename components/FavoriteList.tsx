...
export default function FavoriteList({ favorites, onSelect }: Props) {
	...
	return (
		<div>
			<p className="text-lg font-bold text-slate-100 mb-4">Favorites</p>
			<div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
				{favorites.map((item, i) => (
					...
					>
						<div className="relative w-20 h-20 rounded-2xl overflow-hidden bg-slate-800 group-hover:-translate-y-0.5 transition">
							{item.imageUrl && (
								<Image
									src={item.imageUrl}
									alt={item.name}
