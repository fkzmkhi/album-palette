...
export default function Home() {
  ...
  const handlePaletteReady = (palette: Palette) => {
    if (!selected) return;
    const item: HistoryItem = { ...selected, palette, visitedAt: new Date().toISOString() };
    addToHistory(item);
    setHistory(getHistory());
  };
    ...
    <main className="min-h-screen bg-zinc-950 text-white px-4 py-10">
      <div className="mx-auto max-w-3xl flex flex-col gap-8">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-bold tracking-tight">Album Palette</h1>
          <p className="text-white/40 text-sm">アルバムジャケットからカラーパレットを抽出</p>
        </div>
